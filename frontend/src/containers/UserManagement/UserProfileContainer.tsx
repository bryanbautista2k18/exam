...
import UserProfileForm from "../../components/UserManagement/UserProfileForm.tsx";
...
const UserProfileContainer = memo((): React.JSX.Element => {
  ...
  const [input, setInput] = useState<InputUserType>({});
  const [inputError, setInputError] = useState<InputUserErrorType>({});
  const [configGenders, setConfigGenders] = useState<{ [key: string]: unknown }[]>([]);
  ...
  const getTableData = useCallback(async (): Promise<void> => {
    if (!loading) {
      try {
        setLoading(true);
        await apiService.get(`${location.pathname}?search_key=${searchKey}&page=${currentPage}&limit=${pageLimit}${!configGenders.length ? `&genders=all` : ``}`)
          .then(({ data }) => {
            populateTableData(data.data.users);
            if (data.data.configGenders?.length) {
              setConfigGenders(data.data.configGenders);
            }
          })
          .catch((error) => {
  ...
  }, [loading, searchKey, currentPage, pageLimit, configGenders]);
  ...
  const populateTableData = useCallback((users: { [key: string]: unknown }[]): void => {
    setShow({ card: true, form: false });
    setTableData(users);
    setHasMorePages(users.length === pageLimit);
  }, [pageLimit]);
  ...
  const handleChangeEvent = useCallback((e: React.ChangeEvent): void => {
    const currentField = (e.target as (HTMLInputElement|HTMLSelectElement));
        ...
        await apiService.get(`${location.pathname}/${params.id}`)
          .then(({ data }) => {
            setInput({
              ...
              configGenderId: data.data.user.configGender.id,
  ...
  return (
    ...
    ? (
        <>
          ...
          <div className="card">
            <CardHeader name="User" pluralizedName="Users" show={show} setShow={setShow} />
                ...
                : (
                    <>
                      ...
                      <div className="table-responsive">
                        <table className="table table-sm table-striped user-table">
